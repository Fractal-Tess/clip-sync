...
impl DaemonState {
    ...
    pub(super) async fn handle(&self, request: Request) -> Response {
        ...
        let body = match request.body {
            ...
            Some(request::Body::PeerInterfacesUpdate(update)) => {
                let (reply, result) = oneshot::channel();
                if self
                    .inner
                    .commands
                    .send(DaemonCommand::UpdatePeerInterfaces {
                ...
                match result.await {
                    Ok(Ok(())) => response::Body::Mutation(MutationResponse {
                        ok: true,
                        message: "peer interfaces updated".to_owned(),
                        resource_id: Some("peer_interfaces".to_owned()),
                    }),
                    Ok(Err(message)) => {
                        return error_response(request_id, "setting_update_failed", message);
                    }
